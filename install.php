...
if(isset($_SESSION['install'])){
	if($_GET['do'] == 'update'){
		...
		$numcfg = ["{WEBURL}", "{PATH}", "{DB_HOST}", "{DB_USER}", "{DB_PASS}", "{DB_NAME}"];
		$change   = ["{$_POST['weburl']}", "{$_POST['path']}","{$_POST['dbhost']}", "{$_POST['dbuser']}", "{$_POST['dbpass']}", "{$_POST['dbname']}"];
	...
	<div class="ibox-content">
	   <?php if(isset($_SESSION['install'])): ?>
		...
		<div class="form-group">
			<label class="col-sm-2 control-label">Mật khẩu truy cập</label>
		<div class="col-sm-10">
	...
	<div class="ibox-content">
	   <form id="install" method="POST" action="" class="form-horizontal">
			<div class="form-group"><label class="col-sm-2 control-label">Website</label>
				...
				<small>Địa chỉ dẫn đến trang chủ (vui lòng ghi <strong>http://domain.com</strong> chứ không phải http://domain.com/ để tránh bị lỗi)</small></div>
			</div>
			
			<div class="form-group"><label class="col-sm-2 control-label">Path folder</label>
				<div class="col-sm-10"><div class="input-group m-b"><span class="input-group-addon">*/server/</span> <input type="text" name="path" value="<?php echo __PATH__ ?>" placeholder="Path folder" class="form-control"></div>
				<small><strong>Đang là mặt định</strong>. Nơi lưu trữ file của bạn, phải kết thúc bằng dấu / (folder/save/)</small></div>
			</div>
...
$("#install").on('submit',(function(e) {
	...
	$.ajax({
					...
					$.ajax({
						...
						success: function(data) {
							if(data == true)
								...
								swal("Lỗi mySQL!", "Cú pháp lỗi: " + data, "error")
						}
					})
				});
			}
		...
		error: function(){
			...
			$('#isnbtn').text('Đăng nhập').prop('disabled', false)
		}
   });
}));
<?php endif; ?>
